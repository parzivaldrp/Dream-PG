...
function Findpg() {
  ...
  useEffect(() => {
    if (data) {
      const filtered = data.filter(pg => {
        const matchesCategories = filterCriteria.categories ? pg.categories === filterCriteria.categories : true;
        ...
        return matchesCategories && matchesIdealFor && matchesBudget;
      });
  ...
  const isFiltered = filterCriteria.categories || filterCriteria.idealFor || filterCriteria.budget;
  ...
  return (
    <div>
      ...
      <div className={styles.bapuo}>
        <div>
          <Categories onSubmit={handleFilterSubmit} />
        </div>
